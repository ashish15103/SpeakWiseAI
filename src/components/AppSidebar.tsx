...
export function AppSidebar({ user, featureSlot }: Props) {
  ...
  useEffect(() => {
    function handleResize() {
      if (window.innerWidth >= SIDEBAR_BREAKPOINT) {
        // Large/fullscreen → always open
        setIsCollapsed(false);
      } else {
        // Split/smaller screen → collapsed
        setIsCollapsed(true);
        setSearchOpen(false);
  ...
  return (
    <aside
      className={`relative z-20 flex h-screen shrink-0 flex-col overflow-visible bg-white transition-[width] duration-300 ease-in-out dark:bg-gray-950 ${
    ...
    >
      ...
      <SignOutModal
        ...
        onConfirm={() => {
          setIsSignOutOpen(false); // Close the modal
